...
Supports NCCL, NCCLX (via torchcomms), and Gloo backends.

## Requirements

### Basic Setup (NCCL only)
```bash
...
## Usage on SLURM Systems (e.g., Perlmutter)
...
# Check allocated nodes with: echo $SLURM_JOB_NODELIST
ssh nid001234  # Replace with your first node
...
|----------|------|---------|-------------|
| `--backend` | str | `nccl` | Communication backend (`nccl`, `ncclx`, `gloo`) |
| `--tensor-sizes` | int[] | `[1048576, 4194304, 16777216]` | Tensor sizes in elements (1M, 4M, 16M) |
| `--output-dir` | str | `./results` | Directory to save profiling results |

### Examples

Custom tensor sizes:
```bash
srun --ntasks=4 --gpus-per-node=4 python3 micro_profiling.py --tensor-sizes 1048576 8388608 33554432
```

Using NCCLX backend (requires torchcomms):
```bash
srun --ntasks=4 --gpus-per-node=4 python3 micro_profiling.py --backend ncclx
```

Custom output directory:
```bash
srun --ntasks=4 --gpus-per-node=4 python3 micro_profiling.py --output-dir ./my_results
```
