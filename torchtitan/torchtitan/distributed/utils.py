...
def init_distributed(
    ...
    def _get_distributed_backend(enable_cpu_backend):
        backend_override = (comm_config.backend or "auto").lower()
        if backend_override not in ("", "auto"):
            ...
            if isinstance(backend, torch.distributed.Backend):
                backend = backend.value
        if enable_cpu_backend:
            backend_str = backend if isinstance(backend, str) else str(backend)
            if ":" in backend_str or backend_str == "gloo":
                return backend
            backend = f"{device_type}:{backend_str},cpu:gloo"
        return backend
